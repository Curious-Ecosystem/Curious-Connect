...
Create a meeting with a `HTTP request` containing the `API_KEY` sent to CuriousConnect’s server. The response contains a `meeting` URL that can be `embedded` in your client within an `iframe`.
...
api: {
    // app/api
    keySecret: 'curiousconnectsfu_default_secret',
}
```
...
<iframe
    ...
    src="https://curiousconnect.connect/join/your_room_name"
...
<iframe
    allow="camera; microphone; display-capture; fullscreen; clipboard-read; clipboard-write; autoplay"
    src="https://curiousconnect.connect/newroom"
    style="height: 100vh; width: 100vw; border: 0px;"
></iframe>
```
