...
/**
 * CuriousConnect SFU - Server component
 *
 * @link    GitHub: https://github.com/thecuriousteam/curiousconnectsfu
 * @link    Official Live demo: https://curiousconnect.connect
 * @license For open source use: AGPLv3
 * @license For commercial or closed source, contact us at license.curiousconnect@gmail.com or purchase directly via CodeCanyon
 * @license CodeCanyon: https://codecanyon.net/item/curiousconnect-sfu-webrtc-realtime-video-conferences/40769970
 * @author  Curious Community - info@curiousdevelopers.in
 * @version 1.3.0
 *
 */
...
const defaultStats = {
    enabled: true,
    src: 'https://stats.curiousconnect.com/script.js',
...
function startServer() {
    ...
    app.get('/join/', (req, res) => {
        if (Object.keys(req.query).length > 0) {
            ...
            // http://localhost:3010/join?room=test&roomPassword=0&name=curiousconnectsfu&audio=1&video=1&screen=0&hide=0&notify=1
            // http://localhost:3010/join?room=test&roomPassword=0&name=curiousconnectsfu&audio=1&video=1&screen=0&hide=0&notify=0&username=username&password=password
    ...
    app.post(['/api/v1/meeting'], (req, res) => {
        ...
        if (!api.isAuthorized()) {
            log.debug('CuriousConnect get meeting - Unauthorized', {
                header: req.headers,
                body: req.body,
            });
        ...
        log.debug('CuriousConnect get meeting - Authorized', {
            header: req.headers,
            body: req.body,
            meeting: meetingURL,
        });
    ...
    app.post(['/api/v1/join'], (req, res) => {
        ...
        let api = new ServerApi(host, authorization);
        if (!api.isAuthorized()) {
            log.debug('CuriousConnect get join - Unauthorized', {
                header: req.headers,
                body: req.body,
            });
            return res.status(403).json({ error: 'Unauthorized!' });
        ...
        // log.debug the output if all done
        log.debug('CuriousConnect get join - Authorized', {
            header: req.headers,
            body: req.body,
            join: joinURL,
        });
